...
const FEATURE_GROUPS = [
  ...
  },
  {
    ...
    tools: [
      ...
      { href: '/interview-simulator', icon: '🏆', label: 'Interview Sim', desc: 'Full AI scoring' },
    ],
  },
  {
    group: '🤖 AI Tools',
...
export default function Dashboard() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      {!isSubscribed && !trialExpired && trialDaysLeft <= 5 && (
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              ...
              <span style={{ color: '#94a3b8', fontSize: 13, marginLeft: 8 }}>
                Subscribe now to keep all your data and access.
              </span>
            </div>
          </div>
          <Link href="/subscription" style={{
            ...
            color: '#000', textDecoration: 'none', fontSize: 13, fontWeight: 800,
            whiteSpace: 'nowrap',
          }}>
            Subscribe Now →
          </Link>
      ...
      {/* ── TRIAL ACTIVE SOFT BANNER (day 1–9) ───────────── */}
      {!isSubscribed && !trialExpired && trialDaysLeft > 5 && (
        <div style={{
          ...
          <span style={{ fontSize: 13, color: '#6ee7b7' }}>
            🎉 Free trial active — <strong>{trialDaysLeft} days</strong> remaining. Enjoying EduLink?
          </span>
          <Link href="/subscription" style={{ fontSize: 12, color: '#10b981', textDecoration: 'none', fontWeight: 700 }}>
            View Plans →
          </Link>
        </div>
      )}
      ...
      <div style={{ maxWidth: 1140, margin: '0 auto', padding: '40px 24px 80px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 32 }}>
          ...
          <div>
            <h1 style={{ fontSize: 26, fontWeight: 800, color: '#f1f5f9', marginBottom: 3 }}>
        ...
        {!profile?.bio && (
          <div style={{
            ...
            <Link href="/profile" style={{ padding: '9px 20px', borderRadius: 9, background: '#f59e0b', color: '#000', fontWeight: 700, fontSize: 13, textDecoration: 'none' }}>
              Complete Profile →
            </Link>
          </div>
        )}
        ...
        <div style={{
          ...
          <Link href="/assistant" style={{ padding: '10px 22px', borderRadius: 10, background: 'linear-gradient(135deg,#7c3aed,#2563eb)', color: '#fff', fontWeight: 700, fontSize: 13, textDecoration: 'none', whiteSpace: 'nowrap' }}>
            Chat with EduBot →
          </Link>
        </div>
        ...
        <div style={{ marginBottom: 36 }}>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            {[
              ...
              { href: '/internships', label: '💼 Find Internships', color: '#10b981' },
              { href: '/scholarships', label: '🎓 Scholarships', color: '#f59e0b' },
              { href: '/mock-interview', label: '🎙️ Mock Interview', color: '#ef4444' },
              { href: '/salary-explorer', label: '💰 Salary Rates', color: '#06b6d4' },
              { href: '/tracker', label: '📊 Applications', color: '#8b5cf6' },
              { href: '/notifications', label: '🔔 Notifications', color: '#f97316' },
              { href: '/nearby', icon: '📍', label: 'Near Me', desc: 'Local opportunities', color: '#10b981' },
              { href: '/classroom', icon: '🏫', label: 'Teaching Hub', desc: 'All learning tools', color: '#2563eb' },
              { href: '/address-finder', icon: '🗺️', label: 'Address Finder', desc: 'AI conversation finder', color: '#7c3aed' },
              { href: '/library', icon: '📚', label: 'Library', desc: 'Collected files + AI tutor', color: '#7c3aed' },
            ].map(q => (
              <Link key={q.href} href={q.href} style={{
                padding: '9px 18px', borderRadius: 10,
                background: `${q.color}15`,
                border: `1px solid ${q.color}30`,
                color: q.color, textDecoration: 'none',
        ...
        {/* ── COMMUNITY LINKS ───────────────────────────────── */}
        <div style={{ marginTop: 36, display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: 12 }}>
          {[
